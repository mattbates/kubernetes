...
The Flocker Control Service should be installed manually on a host outside (but accessible from) the Kubernetes cluster. In the future, this may be deployed in pod(s) and exposed as a Kubernetes service.

#### Flocker Agent(s)

The Flocker Agents should be manually installed on *all* Kubernetes nodes. These agents are responsible for provisioning, (de)attachment and (un)mounting and are therefore services that should be run with appropriate privileges on these hosts.
...
The following *volume* spec in  illustrates a sample configuration using a nginx pod with a www-root that uses the volume.
...
- **datasetName** is the unique name for the Flocker dataset.

If the dataset does not already exist in Flocker, it is automatically provisioned, attached and mounted by the plugin. Otherwise, with an existing dataset, it is reattached by Flocker to the node that the pod is scheduled.

**Note**: by default, a new dataset will be provisioned with a standard size of 100GiB. If any other size is required, at this stage the volume should be provisioned manually using the Flocker tools (e.g. [Volumes CLI](https://docs.clusterhq.com/en/latest/labs/volumes-cli.html)).
