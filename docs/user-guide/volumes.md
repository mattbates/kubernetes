...
A `flocker` volume allows a Flocker dataset to be mounted into a pod. If the dataset does not already exist in Flocker, it is automatically provisioned, attached and mounted. Otherwise, with an existing dataset, it is reattached by Flocker to the node that the pod is scheduled. This means data can be "handed off" between pods as required.
